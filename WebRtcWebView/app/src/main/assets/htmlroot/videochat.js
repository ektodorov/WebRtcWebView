...
function startCall() {
    console.log("startCall click");
    //var signalingWebsocket = new WebSocket("ws://" + window.location.host + "/video-conf-tutorial/signal");
    signalingWebsocket = new WebSocket("ws://192.168.1.102:8080/video-conf-tutorial/signal");
...
async function displayLocalStreamAndSignal(firstTime) {
    console.log('displayLocalStreamAndSignal start');
    const localVideo = document.getElementById('localVideo');
    let localStream;
    try {
    ...
    console.log('displayLocalStreamAndSignal end');
}
